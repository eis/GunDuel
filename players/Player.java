...
public abstract class Player {
    ...
    public Player(int opponent) { mOpponent = opponent; mAmmo = 0; }
    ...
    public Vector<Action> getHistory() {return mHistory;}
    // Get History of opponent.
    public Vector<Action> getHistoryOpponent() { return mHistoryOpponent;}
    
    
    // Action '0': load ammo
    protected Action load() { mAmmo += 1; return action(Action.LOAD); }

    // Action '1': fire bullet
    protected Action bullet() { mAmmo -= 1; return action(Action.BULLET); }

    // Action '2': fire plasma
    protected Action plasma() { mAmmo -= 2; return action(Action.PLASMA); }

    // Action '-': shoot bullet
    protected Action metal() { return action(Action.METAL); }

    // Action '=': shoot bullet
    protected Action thermal() { return action(Action.THERMAL); }

    // Get the unique identifier of opponent.
    protected int getOpponent() { return mOpponent; }

    // Get ammo count of opponent.
    protected int getAmmoOpponent(){ return mAmmoOpponent; }
